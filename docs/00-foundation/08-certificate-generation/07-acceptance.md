...
- [ ] Cert download endpoint returns 302 redirect to a 24h-signed Supabase Storage URL.
- [ ] Admin can regenerate a certificate; the same `certificateId` is reused and the object is overwritten.
- [ ] Generation is idempotent — re-running with the same `enrollmentId` produces no duplicate object, no duplicate cert ID.
...
1. **Full path:** create completed enrollment → assert `cert_url` set, object exists, SMS+email enqueued.
2. **Idempotency:** trigger generation twice for the same enrollment → only one cert ID, one upload.
3. **Re-generation:** admin POST `/admin/v1/enrollments/:id/regenerate-cert` → object overwritten, `cert_generated_at` updated, `certificate_id` unchanged.
