...
- **[medium] No fieldset pattern.** Per [memory: Forms use daisyUI fieldset pattern](C:/Users/brand/.claude/projects/g--code--wizeworks-AGCONN/memory/feedback_fieldset_pattern.md) and [apps/CLAUDE.md](apps/CLAUDE.md), every input wraps in `<fieldset className="fieldset">`. Profile form uses simple labels.
...
- **[low] Brand wordmark is a small text "AGCONN" in the sidebar.** Per [memory: Tierra is civic-utilitarian, not editorial](C:/Users/brand/.claude/projects/g--code--wizeworks-AGCONN/memory/feedback_tierra_not_editorial.md), it's acceptable; verify against [docs/brand/09-logo.md](docs/brand/09-logo.md) (currently pending).
...
- **`starts_on` localized via `Intl.DateTimeFormat`** with `timeZone: 'UTC'` so the label matches the underlying date-only column. Share-copy for SMS uses translation keys with placeholders, not inline EN strings. (WhatsApp share deferred with the channel.)
