...
export function PlacementReportBuilder({
  ...
  const exportHref = () => {
    ...
    return `/api/reports/placement/export?${sp.toString()}`;
  };
  ...
  return (
    <div className="bg-base-100 border-base-300 rounded-box border p-5">
      ...
      <fieldset className="fieldset mt-4 w-full min-w-0">
        <legend className="fieldset-legend">Training funders</legend>
        <div className="flex flex-wrap gap-2">
          {funders.map((f) => (
            <button
              key={f}
      ...
      <fieldset className="fieldset mt-4 w-full min-w-0">
        <legend className="fieldset-legend">Email delivery (optional)</legend>
        <input
          type="email"
          className="input"
        ...
        />
        <p className="label">
          Leave blank to download immediately. Provide an address to send the report
          with a 24-hour signed download link.
        </p>
      </fieldset>

      <div className="mt-5 flex items-center gap-3">
        <a className="btn btn-primary rounded-full" href={exportHref()}>
          {defaults.email ? 'Send by email' : 'Download export'}
        </a>
        {pending ? (
