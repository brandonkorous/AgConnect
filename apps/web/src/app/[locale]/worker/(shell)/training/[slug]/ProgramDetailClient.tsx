...
import { useProgramSuspense } from '@/lib/api/hooks/training';

function ProgramDetailInner({ locale, slug }: { locale: string; slug: string }) {
  const t = useTranslations('worker.training_hub.detail');
  const { data } = useProgramSuspense(slug);
  if (!data) notFound();
...
export function ProgramDetailClient({ locale, slug }: { locale: string; slug: string }) {
  return (
    <Suspense fallback={<SkeletonCard rows={8} />}>
      <ProgramDetailInner locale={locale} slug={slug} />
    </Suspense>
  );
}
