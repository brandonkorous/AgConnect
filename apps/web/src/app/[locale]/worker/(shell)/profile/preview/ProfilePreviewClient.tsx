...
function ProfilePreviewInner({ locale }: { locale: string }) {
  const t = useTranslations('worker.profile.preview');
...
export function ProfilePreviewClient({ locale }: { locale: string }) {
  return (
    <Suspense fallback={<SkeletonCard rows={8} />}>
      <ProfilePreviewInner locale={locale} />
    </Suspense>
  );
}
