...
import { useApplicationSuspense } from '@/lib/api/hooks/applications';
...
function ApplicationDetailInner({ id }: { id: string }) {
  ...
  const t = useTranslations('worker.application');
  const tEvent = useTranslations('worker.application.detail.event');
  const { data } = useApplicationSuspense(id);
        ...
        <div>
          ...
          <div className="mt-8">
            ...
            <ol className="border-base-300 grid gap-3 rounded-2xl border bg-white p-5">
              {events.map((e) => (
                <li key={e.id} className="flex items-start gap-3 text-[13.5px]">
                  <div className="bg-primary mt-1.5 h-2 w-2 shrink-0 rounded-full" />
                  <div className="min-w-0 flex-1">
                    <div className="font-medium">{tEvent(e.toStatus)}</div>
