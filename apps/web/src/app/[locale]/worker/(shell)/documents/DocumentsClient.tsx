...
import { useWalletSuspense, type WalletItem } from '@/lib/api/hooks/wallet';

function DocumentsInner({ locale }: { locale: string }) {
  const t = useTranslations('worker.documents');
  const tComp = useTranslations('worker.documents.completeness');
  const tGroup = useTranslations('worker.documents.group');
  const tStatus = useTranslations('worker.documents.status');
  const tShares = useTranslations('worker.documents.shares');

  const { data: profile } = useProfileSuspense();
  const { data: wallet } = useWalletSuspense();

  const tasks = [
    { key: 'identity', done: Boolean(profile.firstName && profile.lastName) },
    { key: 'location', done: Boolean(profile.zipCode || profile.county) },
    { key: 'skills', done: profile.skills.length > 0 },
    {
      key: 'experience',
      done:
