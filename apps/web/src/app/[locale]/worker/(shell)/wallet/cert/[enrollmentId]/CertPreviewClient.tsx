...
import { useCertSuspense } from '@/lib/api/hooks/wallet';

function CertPreviewInner({
  locale,
  enrollmentId,
}: {
  locale: string;
  ...
  const t = useTranslations('worker.wallet.cert');
  const { data: cert } = useCertSuspense(enrollmentId);
  if (!cert) notFound();
...
export function CertPreviewClient({
  locale,
  enrollmentId,
}: {
  locale: string;
  ...
  return (
    <Suspense fallback={<SkeletonCard rows={6} />}>
      <CertPreviewInner locale={locale} enrollmentId={enrollmentId} />
    </Suspense>
  );
}
