...
import { useMyPaySuspense, type Paystub } from '@/lib/api/hooks/pay';
import { buildPaystubsCsv, downloadBlob } from '@/lib/export/client-download';

function exportPaystubs(paystubs: Paystub[]): void {
  downloadBlob('agconn-paystubs.csv', buildPaystubsCsv(paystubs), 'text/csv;charset=utf-8');
}

function PayInner({ locale }: { locale: string }) {
  const t = useTranslations('worker.pay');
  const { data } = useMyPaySuspense();
  const { paystubs, summary } = data;
  ...
  const stats = [
    ...
    },
    {
      ...
      value: nextDeposit ? `$${(nextDeposit.netCents / 100).toFixed(2)}` : '—',
      sub: nextDeposit
    ...
    <div className=" px-5 pb-16 pt-8">
      <WorkerPageHeader
        ...
        right={
          <button
            type="button"
            onClick={() => exportPaystubs(paystubs)}
      ...
      <div className="mt-5 grid grid-cols-1 gap-3.5 lg:grid-cols-3">
        ...
        <TaxDocsCard onExport={() => exportPaystubs(paystubs)} />
...
function DirectDepositCard({ locale }: { locale: string }) {
  const t = useTranslations('worker.pay.direct_deposit');
  return (
...
function TaxDocsCard({ onExport }: { onExport: () => void }) {
  const t = useTranslations('worker.pay.tax_docs');
  return (
    <div className="border-base-300 bg-base-100 rounded-2xl border p-[18px]">
      <div className="text-base-content/60 font-mono text-xs font-semibold uppercase tracking-[0.18em]">
        {t('eyebrow')}
      ...
      <button
        type="button"
        onClick={onExport}
        className="text-primary mt-3 inline-flex items-center gap-1.5 bg-transparent text-[12px] font-bold"
      >
        {t('export')}
      </button>
    </div>
...
function WageTransparencyCard({ locale, avgHourly }: { locale: string; avgHourly: string }) {
  const t = useTranslations('worker.pay.wage');
  return (
    <div className="border-base-300 bg-base-100 rounded-2xl border p-[18px]">
      <div className="text-base-content/60 font-mono text-xs font-semibold uppercase tracking-[0.18em]">
        {t('eyebrow')}
...
export function PayClient({ locale }: { locale: string }) {
  return (
    <Suspense fallback={<SkeletonCard rows={8} />}>
      <PayInner locale={locale} />
    </Suspense>
  );
}
