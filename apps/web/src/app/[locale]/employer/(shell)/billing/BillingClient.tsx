...
import {
  ...
  useFounderSlotsSuspense,
} from '@/lib/api/hooks/employer';
import {
  PLAN_FEATURES,
  PLAN_DISPLAY_PRICE,
  type EmployerPlanTier,
  type PriceCohort,
} from '@agconn/schemas';
...
function BillingInner() {
  ...
  const { data: founderSlots } = useFounderSlotsSuspense();
  ...
  return (
    <div className=" px-5 pb-16 pt-8">
      <div className="mb-7 flex flex-wrap items-end justify-between gap-4">
        ...
        {plan !== 'free' && <CheckoutButton mode="portal" label={t('manage_billing')} />}
      </div>
  ...
  return (
    ...
    >
      <div className="card-body">
        ...
        <ul className={['flex flex-col gap-2 text-sm', isEnterprise ? 'mt-2' : 'mt-5'].join(' ')}>
          {featureLines.map((line) => (
            <li key={line} className="flex items-center gap-2">
              <FontAwesomeIcon icon={faCheck} className="text-success h-3 w-3 shrink-0" />
