...
import { useEmployerProfileSuspense } from '@/lib/api/hooks/employer';
import {
...
function ComplianceAuditInner() {
  const locale = useLocale();
  const t = useTranslations('employer.compliance.audit');
  const tc = useTranslations('employer.compliance');
    ...
    <div className="bg-white text-black min-h-screen">
      ...
      <article className="doc-binder mx-auto max-w-[7in] px-[0.5in] pb-[0.75in] pt-[0.5in] print:max-w-none print:px-0 print:pt-0 print:pb-0">
        <header>
          ...
          <h1 className="mt-5 text-[22pt] font-semibold leading-tight">{t('title')}</h1>
          ...
          <div className="mt-5 border-t border-black pt-3">
            ...
            <Field label={t('generated')} value={generatedDate} valueMono />
            <Field label={t('overall_score')} value={`${overall}%`} />
            <Field label={t('actions_due')} value={String(actions.length)} />
          </div>
        </header>
        ...
        {cats.map((c, idx) => (
          <Section key={c.key} number={idx + 2} heading={tc(`category.${c.key}`)}>
            <div className="mt-1 flex items-baseline justify-between">
              <div className="meta">{t('category_score', { score: c.score })}</div>
            </div>
...
function EvidenceIndex({
  ...
  cats: EvidenceCat[];
  emptyLabel: string;
  labels: { item: string; kind: string; evidence: string; kindFile: string; kindUrl: string };
}) {
  ...
  if (rows.length === 0) return <p className="mt-2">{emptyLabel}</p>;
...
function buildDocId(seed: string, today: Date): string {
  const tag = seed.toUpperCase().replace(/[^A-Z]/g, '').slice(0, 8) || 'AGCONN';
  const ymd =
...
export function ComplianceAuditClient() {
  return (
    <Suspense fallback={<SkeletonCard rows={6} />}>
      <ComplianceAuditInner />
    </Suspense>
  );
}
