...
import { useEmployerInboxSuspense } from '@/lib/api/hooks/employer';
import { ApplicantActions } from '@/components/employer/ApplicantActions';
import { SkeletonCard } from '@/components/ui/skeleton';

function ApplicantDetailInner({ id }: { id: string }) {
...
export function ApplicantDetailClient({ id }: { id: string }) {
  return (
    <Suspense fallback={<SkeletonCard rows={6} />}>
      <ApplicantDetailInner id={id} />
    </Suspense>
  );
}
