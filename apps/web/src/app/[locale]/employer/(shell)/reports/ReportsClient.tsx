...
import { useReportsOverviewSuspense } from '@/lib/api/hooks/employer-ops';
import { useEmployerJobsSuspense, type EmployerJobView } from '@/lib/api/hooks/employer';
...
function ReportsInner() {
  ...
  const range = normalizeRange(sp.get('range') ?? undefined);
  const t = useTranslations('employer.reports');
  const { data } = useReportsOverviewSuspense();
    ...
    <div className=" px-5 pb-16 pt-8">
      ...
      <div className="stats stats-vertical lg:stats-horizontal bg-base-100 border-base-300 mb-6 w-full rounded-2xl border shadow-[var(--shadow-card)]">
        {data.kpis.map((k) => {
          ...
          const isEmpty =
            k.value == null || k.value === '' || k.value === '0' || k.value === '—';
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              ...
              <div>
                ...
                <div className="bg-base-200 mt-1 h-1.5 overflow-hidden rounded-full">
                  <div
                    className={[
                      'h-full',
                      j.fillPct === 100 ? 'bg-success' : j.fillPct > 50 ? 'bg-accent' : 'bg-error',
                    ].join(' ')}
        ...
        <div className="bg-base-100 border-base-300 rounded-2xl border p-6">
          ...
          {data.topWorkers.length === 0 && (
            <div className="text-base-content/55 py-4 text-sm">{t('top_workers_empty')}</div>
          )}
...
type SeasonRange = { label: string; year: number; monthLabels: string[] };
...
function FlowChart({ points }: { points: { week: number; applied: number; hired: number }[] }) {
  const maxV = 200;
  const w = 1100;
  const h = 240;
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="h-60 w-full">
      {[0, 60, 120, 180, 240].map((y) => (
        <line key={y} x1="0" y1={y} x2={w} y2={y} stroke="var(--color-base-300)" strokeWidth="1" />
      ))}
      {points.map((p, i) => {
