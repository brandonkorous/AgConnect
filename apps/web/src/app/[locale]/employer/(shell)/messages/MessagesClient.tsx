...
function MessagesInner() {
  ...
  const folder: FolderKey =
    folderParam && (VALID_FOLDERS as ReadonlyArray<string>).includes(folderParam)
      ? (folderParam as FolderKey)
      : 'all';
  const t = useTranslations('employer.messages');
  const { data: threadsData } = useThreadsSuspense(folder);
  const { threads, counts } = threadsData;
  const requestedThreadId = sp.get('thread');
  const requestedThread = requestedThreadId
    ? threads.find((th) => th.id === requestedThreadId)
    : null;
  const activeThread = requestedThread ?? threads[0] ?? null;
  const { data: messages } = useMessagesSuspense(activeThread?.id ?? null);
...
function FoldersColumn({
  ...
  const folders: Array<{ key: FolderKey; icon: typeof faComments; dot?: boolean }> = [
...
function ThreadList({
  ...
  return (
    <div className="border-base-300 overflow-y-auto border-r grow-1">
      ...
      {threads.length === 0 && (
        <div className="text-base-content/60 p-6 text-center text-xs">{t('empty')}</div>
      )}
      {threads.map((th) => {
...
function Conversation({
  ...
  const initialChannel: 'app' | 'sms' = thread.channel === 'sms' ? 'sms' : 'app';
  return (
