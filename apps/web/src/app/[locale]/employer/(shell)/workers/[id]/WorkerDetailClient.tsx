...
function WorkerDetailInner({ id }: { id: string }) {
  ...
  return <WorkerDetailBody id={id} locale={locale} t={t} />;
}
...
function WorkerDetailBody({
  ...
  const { data: worker } = useWorkerDetailSuspense(id);
  const { data: jobs } = useEmployerJobsSuspense();
  if (!worker) notFound();
  const activeJobs = jobs.filter((j) => j.status === 'active');
  const alreadyInvited =
    worker.relationship === 'invited' || worker.relationship === 'hired';
...
function Section({ heading, children }: { heading: string; children: React.ReactNode }) {
  return (
    <div className="mt-6">
      <h2 className="text-base-content/60 font-mono text-xs uppercase tracking-wider">{heading}</h2>
