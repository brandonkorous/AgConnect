...
import { useWageStatementSuspense } from '@/lib/api/hooks/employer-ops';
...
function WageStatementInner({ periodId, lineId }: { periodId: string; lineId: string }) {
  const locale = useLocale();
  const t = useTranslations('employer.payroll.wage_statement');
  const { data } = useWageStatementSuspense(periodId, lineId);
  if (!data) notFound();
...
export function WageStatementClient({
  periodId,
  lineId,
}: {
  periodId: string;
  ...
  return (
    <Suspense fallback={<SkeletonCard rows={6} />}>
      <WageStatementInner periodId={periodId} lineId={lineId} />
    </Suspense>
  );
}
