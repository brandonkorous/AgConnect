...
export function SectionNav({ memberCount }: { memberCount: number }) {
  const t = useTranslations('employer.crews.edit_crew.section');
  ...
  return (
    <nav aria-label={t('nav_aria')} className="flex flex-col gap-0.5">
      {SECTION_IDS.map((id) => {
        const on = active === id;
        const label = id === 'roster' ? t('item.roster_count', { count: memberCount }) : t(`item.${id}`);
        return (
          <a
            key={id}
            href={`#${id}`}
