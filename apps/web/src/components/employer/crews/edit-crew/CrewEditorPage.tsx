...
export function CrewEditorPage({
    ...
    const [archiveTypedConfirm, setArchiveTypedConfirm] = useState('');
        ...
        <div className=" px-5 pb-16 pt-8">
            ...
            {archiveOpen && crew && (
                <div className="bg-base-200/60 border-base-300 mb-5 rounded-2xl border p-4">
                    ...
                    <fieldset className="fieldset mt-3 w-full min-w-0">
                        <legend className="fieldset-legend text-xs">
                            {t('archive_typed_confirm_label', { name: crew.name })}
                        </legend>
                        <input
                            type="text"
                            className="input input-bordered input-sm w-full"
                            value={archiveTypedConfirm}
                            onChange={(e) => setArchiveTypedConfirm(e.target.value)}
                            placeholder={crew.name}
                            autoComplete="off"
                            spellCheck={false}
                            aria-describedby="archive-typed-confirm-hint"
                        />
                        <p id="archive-typed-confirm-hint" className="label">
                            {t('archive_typed_confirm_hint')}
                        </p>
                    </fieldset>
                    <div className="mt-3 flex justify-end gap-2">
                        ...
                        <button
                            ...
                            disabled={
                                busy ||
                                archiveTypedConfirm.trim().toLowerCase() !== crew.name.trim().toLowerCase()
                            }
