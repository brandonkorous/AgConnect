  ...
  return (
    <div className="mb-6 grid gap-3.5 sm:grid-cols-2 lg:grid-cols-4">
      {tiles.map((tile) => (
        ...
        >
          <div className="text-base-content/60 font-mono text-[11px] font-semibold uppercase tracking-[0.18em]">
            {t(`${tile.key}_label`)}
          </div>
          <div className="mt-2.5 flex items-end gap-2.5">
            <div
              className={[
                'font-display text-4xl font-light leading-none tracking-tight',
                tile.accent === 'primary'
                  ? 'text-primary'
                  : tile.accent === 'accent'
                    ? 'text-accent'
            ...
            {tile.delta && (
              <div
                className={[
                  'pb-1.5 font-mono text-xs font-bold',
                  tile.delta.tone === 'good' ? 'text-success' : 'text-warning',
                ].join(' ')}
              >
                {tile.delta.value}
              </div>
          ...
          <div className="text-base-content/60 mt-2 text-xs">{tile.sub}</div>
        </div>
      ))}
    </div>
  );
