...
async function handleSmsJob<T extends SmsTemplateName>(job: Job<SmsJob<T>>): Promise<void> {
    ...
    await withTenantTx(tenantId, async (db) => {
        ...
        if (!user || !user.phone) {
            console.warn('[sms-worker] dropped: no phone', { userId, template });
        ...
        // Must use db (inside withTenantTx) so SET LOCAL app.role = 'service' is
        // active and the sms_opt_out_service RLS policy allows the read.
        // Using the global prisma here would see 0 rows (no app.role set on the
        // connection) and silently bypass the opt-out check — a TCPA violation.
        const optOut = await db.smsOptOut.findUnique({ where: { phone: user.phone } });
        if (optOut) {
            console.warn('[sms-worker] dropped: opted out', { userId, template, toPhone: user.phone });
            const dropped = await db.smsLog.create({
                data: {
                    tenantId,
                    userId,
        ...
        try {
            ...
            console.log('[sms-worker] sent', { logId: log.id, template, providerSid: msg.sid, toPhone: user.phone });
            await db.smsLog.update({
                where: { id: log.id },
            ...
            console.error('[sms-worker] twilio send failed', { logId: log.id, template, toPhone: user.phone, errorCode: cls.code, errorMessage: cls.message, retryable: cls.retryable });
            await db.smsLog.update({
                where: { id: log.id },
